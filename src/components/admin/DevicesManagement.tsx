...
export function DevicesManagement() {
  ...
  const generateFingerprint = () => {
    const fp = `${navigator.userAgent}-${screen.width}x${screen.height}-${new Date().getTimezoneOffset()}-${Math.random().toString(36).substring(2, 15)}`;
    const hash = btoa(fp).substring(0, 32);
    setFormData({ ...formData, fingerprint: hash });
  };
